...
const ExpressionNumber = ({numerologyDetail}) => {
    ...
    const getExpressionNumber = async () => {
      try {
        ...
        const response: AxiosResponse<{message: string; id: string}> =
          await apiService.post('numerology/expressionNumber', {
            name: numerologyDetail.name,
            expressionNumber: String(numerologyDetail.expression),
          });
        setExpressionNumber({
  ...
  useEffect(() => {
    if (numerologyDetail) {
      const uri = getImageForNumber(numerologyDetail.expression);
      setImage(uri);
    }
  }, [numerologyDetail]);
